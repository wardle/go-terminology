...
func TestExpressionConstraint(t *testing.T) {
	...
	if e1.GetRefined().GetRefinement().GetSubrefinement().GetAttributeSet().GetSubattributeSet().GetAttribute().GetSubexpressionValue().GetFocusConcept().GetConceptReference().GetConceptId() != 79654002 {
		t.Fatalf("did not parse attribute value. got %v", e1.GetRefined().GetRefinement())
	}
	...
	ParseConstraint(s3)
	s4 := "< 19829001 |Disorder of lung|  AND <  301867009 |Edema of trunk|"
	ParseConstraint(s4)
	s5 := "<  27658006 |Amoxicillin| : 411116001 |Has dose form|  =  <<  385055001 |Tablet dose form| , { 179999999100 |Has basis of strength|  = ( 219999999102 |Amoxicillin only| : 189999999103 |Has strength magnitude| >= #200,199999999101 |Has strength unit|  =  258684004 |mg| )}"
	ParseConstraint(s5)
}
...
func TestConstraintSelf(t *testing.T) {
	...
	if c1.GetSubexpression().GetFocusConcept().GetConceptReference().GetConceptId() != 404684003 {
		t.Fatalf("did not parse 'self' constraint %s. got %v", s1, c1)
	}
	if c1.GetSubexpression().GetConstraint() != snomed.ExpressionConstraint_SELF {
